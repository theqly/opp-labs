...
void* worker_func(void* void_data){
    ...
    for(int i = 0; i < 12; i++){
        ...
        if(data->rank == 0) printf("disbalace: %lf and dolya: %lf at iteration %d\n", disbalance, dolya_disbalace, i);
...
void* receiver_func(void* void_data){
    struct data* data = (struct data*) void_data;
...
int main(int argc, char** argv){
    ...
    if (pthread_attr_setdetachstate(&attr,PTHREAD_CREATE_JOINABLE) != 0){
        fprintf(stderr, "cant set attr as PTHREAD_CREATE_JOINABLE\n");
        MPI_Finalize();
        return 1;
    }
    ...
    if(pthread_create(&receiver, &attr, receiver_func, data) != 0){
        fprintf(stderr, "error in creating receiver\n");
        MPI_Finalize();
        return 1;
    }
    ...
    if(pthread_join(receiver, NULL) != 0){
        fprintf(stderr, "error in joining receiver\n");
        MPI_Finalize();
        return 1;
    }
